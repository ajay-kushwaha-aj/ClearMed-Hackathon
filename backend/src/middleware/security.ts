...
// ── FIX: realIp — req.ip is a READ-ONLY getter in Express when trust proxy
//    is enabled. Writing to it throws "Cannot set property ip of #<IncomingMessage>
//    which has only a getter" and crashes EVERY request.
//    Since app.set('trust proxy', 1) is already set in index.ts, Express
//    automatically reads X-Forwarded-For and exposes it via req.ip correctly.
//    This middleware is now a safe no-op.
// ─────────────────────────────────────────────────────────────────────────
export function realIp(_req: Request, _res: Response, next: NextFunction) {
  // Do nothing — Express handles this automatically via 'trust proxy' setting.
  next();
}