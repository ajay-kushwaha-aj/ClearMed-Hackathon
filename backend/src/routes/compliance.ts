...
router.post('/erasure-request', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // FIX: was 'BILL_PENDING', ErasureRequest.status default is plain string 'PENDING'
    const existing = await prisma.erasureRequest.findFirst({ where: { userId: user.id, status: 'PENDING' } });
    if (existing) {
    ...
    await prisma.erasureRequest.create({
      data: { userId: user.id, email, reason, status: 'PENDING' },
    });
